...
function contact2vcard($contact, $categories) {
    ...
    if (is_null($conf['vcard-version'])) $conf['vcard-version']='3.0';

    $card = new Contact_Vcard_Build($conf['vcard-version']);
    ...
    foreach($contact->phones as $item) {
	    ...
	    if (($conf['vcard-version']=='2.1') & ($item['label']!='MAIN')) $card->addParam('TYPE', $item['label']);
	    if ($conf['vcard-version']!='2.1') $card->addParam('TYPE', $item['label']);
    ...
    if(!empty($contact->image) and !empty($conf['photo_format'])) {
		...
		if ($conf['vcard-version']!='2.1') $card->addParam('ENCODING', 'B');
		if ($conf['vcard-version']=='2.1') $card->addParam('ENCODING','BASE64');
