...
//+----------------------------------------------------------------------+
//
// 2006-10-10 <hans@at.or.at>: added fixed from discussion page
//
// $Id$
	...
	*/
	function setRecEnd($freq = '') {
		if (strlen(trim($freq)) < 1) {
			$this->rec_end = 0;
		} elseif (is_numeric($freq) && strlen(trim($freq)) < 4) {
			$this->rec_end = $freq;
		} else {
	...
	*/
	function setStartDate($timestamp = 0) {
        if (preg_match('/^(\d{4})-(\d{2})-(\d{2})$/',$timestamp,$matches)) {
            $this->startdate = (string) $matches[1].$matches[2].$matches[3];
            return;
        }
	...
	*/
	function setUID($uid = 0) {
		if (strlen(trim($uid)) > 0) {
            ...
            $rawid = (string) $this->startdate . 'plus' .  $this->enddate . $this->summary;
