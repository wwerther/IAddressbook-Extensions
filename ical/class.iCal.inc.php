		...
		if ($this->output_format == 'ics') {
			...
			foreach ($eventkeys as $id) {
				...
				if ($event->getFrequency() > 0) {
					...
					$this->output .= (string) ";INTERVAL=" . $event->getInterval() ;
                    if ($event->getDays())  $this->output .= ";BYDAY=" . $event->getDays() ; 
                    $this->output .= ";WKST=" . $event->getWeekStart() ;
                    $this->output .= "\r\n";
				} // end if
				...
				if (!isEmpty($event->getDescription())) {
					$this->output .= (string) "DESCRIPTION" . $event->getLanguage() . ";ENCODING=QUOTED-PRINTABLE;CHARSET=UTF-8:" . str_replace('\n', '=0D=0A=',str_replace('\r', '=0D=0A=', $this->quotedPrintableEncode($event->getDescription()))) . "\r\n";
				} // end if
				$this->output .= (string) "SUMMARY" . $event->getLanguage() . ";ENCODING=QUOTED-PRINTABLE;CHARSET=UTF-8:" . $this->quotedPrintableEncode($event->getSummary()) . "\r\n";
